	    	...
	    	<#if user.isGuest == false>
	    		...
	    		<#if ( user.getId() != author.getUser().getId() ) >
		    		<#if pratilipi.isAddedToLib() == true>
		    			<button type="button" class="pratilipi-grey-button pratilipi-padding-7 pratilipi-font-size-14" onclick="AddToLibrary( ${ pratilipi.getId()?c }, false )">- ${ _strings.library } </button>
		    		<#else>
		    			<button type="button" class="pratilipi-light-blue-button pratilipi-padding-7 pratilipi-font-size-14" onclick="AddToLibrary( ${ pratilipi.getId()?c }, true )">+ ${ _strings.library } </button>
		    		</#if>
		    	</#if>	
	    	</#if>
	    </div> 		 
