...
TableOfContents.prototype.populateIndex = function( index ) {
	...
	$.each( this.chaptersList, function( index, chapter ) {
		var chapter = new Chapter( chapter, _this.parent_object );
		_this.chapters.push( chapter );
		chapter.getListDomElement().insertBefore( _this.$chapters_separator_second );
	});
...
TableOfContents.prototype.addNewChapterButton = function() {
	...
	var $boldNewChapter = $( "<strong>", {
		"class": "pratilipi-red"
	}).text( " + ${ _strings.writer_add_new_chapter }" );
