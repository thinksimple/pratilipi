<div class="pratilipi-shadow secondary-500 box">	
	<div class="pull-left">
		...
		<p class="works-number"><!-- {{ numberFound }} ${ _strings.author_count_works } -->1.2k Members</p>
	</div>
	...
	<#if followersList.getUserList()?has_content>
		...
		<#list followersList.getUserList() as local_user>
			...
			<@follow_author_card isGuest=user.isGuest?c can_follow=can_follow_boolean retUrl=author.getPageUrl() authorId=local_author.getId() followCount=local_author.getFollowCount() following=local_author.isFollowing() name=local_user.getDisplayName() pageUrl=local_user.getProfilePageUrl() imageUrl=local_user.getUserImageUrl( 90 )/>
		</#list>
	</#if>		
</div>	